...
type AppEnv = {
  ...
  // The live-event hub — present because "durableObject" is set in the manifest.
  ROOMS?: DurableObjectNamespace;
  HF_ENV?: string;
  APP_SLUG?: string;
  // Real LUXE pipeline (Supabase) — the node map reads leads live from here.
  LUXE_SUPABASE_URL?: string;
  LUXE_SUPABASE_SERVICE_KEY?: string;
  // Auth for /api/jarvis (JARVIS voice assistant's REST access to this dashboard).
