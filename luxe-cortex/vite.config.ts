...
export default defineConfig(({ mode }) => {
  ...
  return {
    // The real Higgsfield platform's design-inspector plugin normally injects
    // this global; ./src/module/design-inspector/vite.ts is a local no-op
    // stub (see its own comment) that doesn't, so __root.tsx's
    // `if (!__HF_DESIGN_INSPECTOR__)` check throws ReferenceError without it.
    define: {
