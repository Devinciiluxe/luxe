    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "job_payload": {
                    "type": "OBJECT",
                    "description": "Payload for queue_job, e.g. {listing_id, body} for send_message",
                    "properties": {
...
def main():
    _check_mindmap_server()
    ui = JarvisUI("face.png")

    def runner():
        ui.wait_for_api_key()

        from core.cortex_session import CortexSession as HermesSession
        ...
        if not api_key:
            ui.write_log(
                "ERR: 'gemini_api_key' is missing in config/api_keys.json."
            )
        ...
        while True:
            try:
                ...
                _backoff = 3  # clean exit — reset backoff
            except KeyboardInterrupt:
                print("\n🔴 Shutting down...")
                break
            except SystemExit:
                ...
                print(f"[JARVIS] Session crashed ({type(e).__name__}): {e} — restarting in {_backoff}s")
                traceback.print_exc()
                time.sleep(_backoff)
