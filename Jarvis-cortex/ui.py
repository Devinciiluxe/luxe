...
class MindMapView(QWebEngineView):
    ...
    def __init__(self, face_path: str, assistant_name: str = "J.A.R.V.I.S", parent=None):
        ...
        # QtWebEngine's Chromium exposes the SpeechRecognition JS API (feature
        # detection passes) but has no backing service wired up — calling it
        # crashes the whole render process ("bad Mojo message: no binder for
        # media.mojom.SpeechRecognizer"), taking the entire window down with
        # it. The CORTEX page's own browser-based voice hook is a leftover
        # for standalone web use anyway — the native app owns all real voice
        # here. Flag this before ANY page script runs so the hook can disable
        # itself instead of ever calling .start().
        flag_script = QWebEngineScript()
        flag_script.setName("jarvis-native-shell-flag")
        flag_script.setSourceCode("window.__JARVIS_NATIVE_SHELL__ = true;")
        ...
        flag_script.setRunsOnSubFrames(True)
        self.page().scripts().insert(flag_script)

        self.setUrl(QUrl(MIND_MAP_URL))
    ...
    @muted.setter
    def muted(self, v: bool): self._muted = v

    @property
    def speaking(self) -> bool: return self._speaking
    @speaking.setter
    def speaking(self, v: bool): self._speaking = v
    ...
    def state(self, v: str):
        self._state = v
        self._run_js(f"window.__jarvisBridge && window.__jarvisBridge.setState({json.dumps(v)});")

    def say(self, text: str) -> None:
        """Mirrors JARVIS's spoken line on the map's JARVIS bar — visual only,
        no browser TTS, since the real voice is already playing from Gemini
        Live through the system speakers."""
        self._run_js(f"window.__jarvisBridge && window.__jarvisBridge.say({json.dumps(text)});")
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("NOUS API KEY", 8, color=C.TEXT_DIM,
                               align=Qt.AlignmentFlag.AlignLeft))
        layout.addWidget(_lbl("Get one at dashboard.nousresearch.com", 8, color=C.TEXT_DIM,
                               align=Qt.AlignmentFlag.AlignLeft))
        self._key_input = QLineEdit()
        self._key_input.setEchoMode(QLineEdit.EchoMode.Password)
        self._key_input.setPlaceholderText("nous-…")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        # `central` (header/side-panels/footer — the OLD chrome) is still
        # built in full below, exactly as before: dozens of methods across
        # this class (write_log, set_state, camera stream, clipboard panel,
        # quick drawer, overlays, the mute button, the text input) all assume
        # these widgets exist. Rather than rip that apart, it's built and
        # then parked off-screen at the end of __init__ — self.hud (the
        # mind-map/camera stack) becomes the real central widget, full
        # takeover, edge to edge. See the setCentralWidget(self._hud_cam_stack)
        # call after the footer is added below.
        central = QWidget()
        ...
        # Center column: talking 3D mind map (CORTEX brain, wired to real
        # leads/revenue data) + resizable content panel via QSplitter
        self.hud = MindMapView(face_path, _display)
        ...
        # Full takeover: hide the old native chrome (header/side-panels/
        # footer, everything just built into `central`/`root`/`body`) and
        # make the mind map (with the live camera-stream swap it already
        # supports via _hud_cam_stack) the entire window instead. `central`
        # stays alive as a real, parented-but-hidden widget so every method
        # that reaches into it later (write_log, mute button, quick drawer,
        # camera preview, etc.) keeps working exactly as before.
        central.setParent(self)
        central.hide()
        self.setCentralWidget(self._hud_cam_stack)
        ...
        self._log_sig.connect(self._sync_mindmap_transcript)
    ...
    def _sync_mindmap_transcript(self, text: str):
        """write_log() lines are formatted '{assistant_name}: {said text}' for
        JARVIS's own turns — mirror just those onto the map's JARVIS bar. ERR:
        lines get surfaced too (as a toast, via .say()) — the old log panel
        that used to show these is now hidden behind the full-window map, so
        without this a startup failure (e.g. a missing API key) fails
        completely silently."""
        prefix = f"{self._assistant_name}: "
        if text.startswith(prefix) and hasattr(self.hud, "say"):
            self.hud.say(text[len(prefix):])
        elif text.startswith("ERR:") and hasattr(self.hud, "say"):
            self.hud.say(text)
    ...
    def _check_config(self) -> bool:
        ...
        try:
            ...
            return bool(d.get("nous_api_key")) and bool(d.get("os_system"))
        except Exception:
            return False
