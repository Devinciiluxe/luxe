...
def _play_audio_bytes(audio_bytes: bytes) -> None:
    """Play MP3 audio bytes via afplay (macOS) — works in foreground and background."""
...
class EdgeTTSEngine:
    ...
    def speak(self, text: str) -> None:
        ...
        if not text:
            return
        print(f"[TTS] synth start: {text[:40]!r}")
        loop = asyncio.new_event_loop()
        try:
            audio_bytes = loop.run_until_complete(self._synth(text))
        finally:
            loop.close()
        print(f"[TTS] synth done: {len(audio_bytes)} bytes, playing…")
        if audio_bytes:
